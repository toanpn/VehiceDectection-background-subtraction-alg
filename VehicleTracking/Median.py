...
while(run):
    ...
    if ret is True:
        ...
        images.append(frame_process)
        # removing the images after every n background
        if len(images)==numberOfImage:
            images.pop(0)

        #declare background
        # background = np.array(images)

        background = np.median(images,axis=0)
        background = background.astype(np.uint8)
        # foreground will be background - curr frame
        cv2.imshow('background',background)
        foreGround = cv2.absdiff(frame_process,background)
